...
Run without arguments → menu:
  1. Single account (interactive email/password prompt)
  2. Combo file (no proxy)
  3. Combo file with proxy rotation
  4. Exit

All outputs go to ./Accor results/
...
def _print_banner() -> None:
    if _RICH:
        ...
        _console.print(Panel(
            ...
            subtitle="[dim]Accor Account Capture · Interactive Menu[/dim]",
            border_style="cyan",
            box=rich_box.DOUBLE,
        ))
        _console.print(Panel(
        ...
        print(Fore.YELLOW + "  ⚡ AKAZA CHECKS — ACCOR CAPTURE (Interactive)" + Style.RESET_ALL)
        print()
...
# ── combo mode core (reused by menu) ────────────────────────
async def _run_combo(combo_file: Path, proxy_list: List[str], args: argparse.Namespace) -> None:
    combos = []
...
async def _run_single(email: str, password: str, args: argparse.Namespace) -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    bag = APIBag()
    ...
    async with async_playwright() as pw:
        ...
        if not use_state:
            if not email or not password:
                print("ERROR: provide email/password", file=sys.stderr)
                await browser.close()
                return
...
async def run_interactive() -> None:
    ...
    while True:
        _sep("MAIN MENU")
        print("  1. Single account (interactive email/password)")
        print("  2. Combo file (without proxy)")
        print("  3. Combo file (with proxy rotation)")
        print("  4. Exit")
        choice = input("\n  Choose [1-4]: ").strip()
        if choice == "1":
            _sep("SINGLE ACCOUNT")
            ...
            if not email or not password:
                _clog("Email and password required", "err")
                continue
            # Create dummy args for single mode
            args = argparse.Namespace(
                show=False, headless=False, fresh=False, keep_open=False, proxy=None
            )
            await _run_single(email, password, args)
        elif choice == "2":
            _sep("COMBO (NO PROXY)")
            combo_path = input("  Path to combo file (email:pass per line): ").strip()
            if not Path(combo_path).is_file():
            ...
            _sep("COMBO WITH PROXY")
            combo_path = input("  Path to combo file: ").strip()
            if not Path(combo_path).is_file():
                _clog(f"File not found: {combo_path}", "err")
                continue
            proxy_path = input("  Path to proxy file (one per line): ").strip()
            if not Path(proxy_path).is_file():
                _clog(f"Proxy file not found: {proxy_path}", "err")
                continue
            ...
            _clog("Invalid choice, try again", "warn")

# ── main entry point ────────────────────────────────────────
async def main() -> int:
    # If any command-line args are given, use original CLI mode (backward compatible)
    if len(sys.argv) > 1:
        parser = argparse.ArgumentParser(description=__doc__.splitlines()[1])
        parser.add_argument("--email", help="Single account email")
        parser.add_argument("--password", help="Single account password")
        parser.add_argument("--combo", help="Combo file")
        parser.add_argument("--proxy", help="Single proxy URL")
        parser.add_argument("--proxy-file", help="File with proxies")
        parser.add_argument("--show", action="store_true", help="Make Chrome visible")
        parser.add_argument("--headless", action="store_true", help="True headless mode")
        ...
        if parsed.combo:
            ...
            if parsed.proxy_file:
                with open(parsed.proxy_file, "r") as f:
                    proxy_list = [l.strip() for l in f if l.strip()]
            elif parsed.proxy:
                proxy_list = [parsed.proxy]
            await _run_combo(Path(parsed.combo), proxy_list, parsed)
        else:
            email = parsed.email or os.environ.get("ACCOR_EMAIL") or ""
            password = parsed.password or os.environ.get("ACCOR_PASSWORD") or ""
            if not email or not password:
                _sep("CREDENTIALS")
                if not email:
            ...
            await _run_single(email, password, parsed)
    else:
